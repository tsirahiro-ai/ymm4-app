...
st.markdown("<p style='text-align: center; color: gray;'>様々な無料高性能AIを自由に切り替えて、霊夢と魔理沙の掛け合い台本を作成・一括ダウンロードできます。</p>", unsafe_allow_html=True)
st.write("") 

# APIキーの入力（サイドバー）
st.sidebar.header("🔑 各種AIのAPIキー設定")
st.sidebar.write("使用したいAIのキーを入力してください（入力がない場合は完全無料のオープンサーバーが自動で使用されます）。")
gemini_key = st.sidebar.text_input("Google Gemini API Key (AIzaSy...)", type="password")
openai_key = st.sidebar.text_input("OpenAI / ChatGPT API Key (sk-...)", type="password")
groq_key = st.sidebar.text_input("Groq API Key (gsk_...)", type="password")
...
if st.button(f"台本を {num_scripts} 本一括生成する"):
    ...
    with st.spinner(f"{num_scripts}本の異なる掛け合いネタを計算中..."):
        # 1. ChatGPTルート
        if "ChatGPT" in ai_provider:
            if not openai_key:
                st.error("OpenAIのAPIキーが入力されていません。サイドバーから入力してください。")
            else:
                try:
        ...
        # 2. Geminiルート
            ...
            if not gemini_key:
                st.error("GeminiのAPIキーが入力されていません。サイドバーから入力してください。")
            else:
                try:
        ...
        # 3. Groqルート
            ...
            if not groq_key:
                st.error("GroqのAPIキーが入力されていません。サイドバーから入力してください。")
            else:
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        st.error(f"Groqエラー: {res.text}")
                except Exception as e:
                    st.error(f"接続エラーが発生しました: {e}")

        # 4. 無料自動マルチサーバールート（Hugging Faceオープンサーバーの3段構え）
        else:
            models = [
            ...
            if not success or not raw_output:
                st.error("一時的にすべての無料AIルートが満席です。30秒ほど後に、もう一度ボタンを押すか、APIキーを入力するルートをお試しください。")
